...
import { collection, getDocs , getDoc , doc, deleteDoc, setDoc, addDoc, query, where, limit } from 'firebase/firestore' 
...
function Home() {
  ...
  //     const collectionReferece = collection(db, 'users')
  //     // addDoc(collectionReferece , {
  //     //   user_name : 'rohit wagh',
  //     //   level : 4
  //     // }).then(response=>{
  ...
  //   // only one user view data 
    
  //   // const getRohitDetails = async () =>{
  //   //   const documentReference = doc(db, 'users', 'Arun');
  ...
  //     // }) 
  //       // deleteDoc(documentReference).then(respo =>{
  //       //   console.log('delete rahul')
  //       // })
  ...
  const firebaseFunction = async()=>{
    ...
    // const q = query(collectionReference , where('category' ,'==','jewelery'))
    // getDocs(q).then(reponse =>{
    //   reponse.forEach(doc=>{
    ...
    getDocs(q).then(reponse =>{
      reponse.forEach(prod =>{
        console.log(prod.data().price , '=>' , prod.data().title)
      })
    })
  }
  firebaseFunction()
...
const handleLogout = () =>{
  ...
  return (
    <div className="home-page">
       <h1 style={{textAlign:"center"}}>Hello Firebase</h1>
